	...
	public static void main(String[] args) throws IOException {
		...
		while(n>0) {
			...
			for(i=0;i<aux.length;i++) {
				if(aux[i]=='P'&&cond) cond = false; 
				if(aux[i]=='Q'&&cond2) cond2 = false; 
				if(aux[i]=='R'&&cond3) cond3 = false; 
				if(aux[i]=='S'&&cond4) cond4 = false; 
			}
			cont = 0;
			if(!cond) {vec.add("P");cont++;}
			if(!cond2) {vec.add("Q");cont++;}
			if(!cond3) {vec.add("R");cont++;}
			if(!cond4) { vec.add("S");cont++;}
			...
			for(i=cont;i<subs.length;i++) {
				...
				for(j=1;j<subs[i].length()-1&&stop;j++) {
					...
					if(subs[i].charAt(j)==')') {balance++;}
					if((subs[i].charAt(j)=='&'||subs[i].charAt(j)=='~'||subs[i].charAt(j)=='>'||subs[i].charAt(j)=='v')&&balance==0){
						stop = false;
					}
				}
				j--;
